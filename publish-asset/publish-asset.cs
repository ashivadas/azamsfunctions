...
namespace publishasset
{
    public class PublishAsset
    {
        static MediaServiceHelper helper = new MediaServiceHelper();
        ...
        public static async Task<HttpResponseMessage> RunAsync([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = "HttpTriggerCSharp/name/{name}")]HttpRequestMessage req, string name, TraceWriter log)
        {
            ...
            try
            {
                ...
                var publishurlsmooth = helper.GetValidOnDemandURI(outputAsset, _context);
                var publishurlpath = helper.GetValidOnDemandPath(outputAsset, _context);
